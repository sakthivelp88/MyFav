...
function AdminPanelOptionsPage() {
  return (
    <section className="space-y-5 rounded-[28px] border border-slate-700/80 bg-[linear-gradient(180deg,rgba(15,23,42,0.98)_0%,rgba(15,23,42,0.92)_100%)] p-6 shadow-[0_28px_80px_rgba(0,0,0,0.38)]">
      <header>
        <h2 className="text-2xl font-bold text-slate-100">Admin Panel Options</h2>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {adminOptions.map((option) => (
          <Link
            key={option.path}
            to={option.path}
            className="group rounded-2xl border border-slate-700 bg-slate-800/85 p-4 transition hover:border-amber-400/60 hover:bg-slate-800"
          >
            <p className="text-lg font-semibold text-slate-100 group-hover:text-amber-300">{option.title}</p>
            <p className="mt-2 text-sm text-slate-400">{option.description}</p>
            <p className="mt-3 text-xs font-semibold uppercase tracking-[0.2em] text-amber-300">Open Page</p>
          </Link>
        ))}
      </div>
    </section>
