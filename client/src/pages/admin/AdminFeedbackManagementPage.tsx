  ...
  return (
    <section className={sectionClass}>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-100">Feedback Management</h2>
          <p className="text-sm text-slate-400">Review what customers are saying and track follow-up status.</p>
        </div>
        <button
          type="button"
      ...
      <article className="rounded-2xl border border-slate-700 bg-slate-800/85 p-4 shadow-lg shadow-black/10">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <button type="button" onClick={() => applyQuickFeedbackFilter('all')} className="rounded-full border border-slate-700 bg-slate-900 px-3 py-1 text-xs font-semibold text-slate-200 hover:bg-slate-700">All Feedback</button>
          <button type="button" onClick={() => applyQuickFeedbackFilter('new')} className="rounded-full border border-amber-500/30 bg-amber-950/20 px-3 py-1 text-xs font-semibold text-amber-300 hover:bg-amber-900/30">New</button>
          <button type="button" onClick={() => applyQuickFeedbackFilter('reviewed')} className="rounded-full border border-sky-500/30 bg-sky-950/20 px-3 py-1 text-xs font-semibold text-sky-300 hover:bg-sky-900/30">Reviewed</button>
          <button type="button" onClick={() => applyQuickFeedbackFilter('resolved')} className="rounded-full border border-emerald-500/30 bg-emerald-950/20 px-3 py-1 text-xs font-semibold text-emerald-300 hover:bg-emerald-900/30">Resolved</button>
          <button type="button" onClick={() => applyQuickFeedbackFilter('rated')} className="rounded-full border border-violet-500/30 bg-violet-950/20 px-3 py-1 text-xs font-semibold text-violet-300 hover:bg-violet-900/30">With Rating</button>
        </div>

        <p className="mt-3 text-xs text-slate-400">Showing {filteredFeedback.length} of {feedback.length} feedback entries.</p>
      </article>

      <div className="space-y-3">
        {paginatedFeedback.map((entry) => (
          <article key={entry._id} className="rounded-2xl border border-slate-700 bg-slate-800/85 p-4 shadow-lg shadow-black/10">
            <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
              <div>
                <p className="font-semibold text-slate-100">{entry.customerName}</p>
                <p className="text-sm text-slate-400">{entry.customerPhone}</p>
        ...
        {!loading && filteredFeedback.length === 0 ? (
          <p className="rounded-2xl border border-slate-700 bg-slate-800 p-4 text-sm text-slate-400">No feedback submitted yet.</p>
        ) : null}
      </div>
