...
const menuLinkClass = 'block rounded-lg px-3 py-2 text-sm text-slate-700 hover:bg-amber-50 hover:text-amber-900'
...
function AppLayout() {
  ...
  const isAdminPage = location.pathname.startsWith('/admin/dashboard') || location.pathname.startsWith('/admin/audit')
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">
                {isScanMode ? 'Scan mode: customer ordering only' : 'Smart QR ordering for customers and staff'}
              </p>
            </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2 rounded-xl border border-slate-200 bg-slate-50 p-2">
              <span
                className={`rounded-lg px-3 py-1 text-xs font-semibold uppercase tracking-wide ${
                  location.pathname.startsWith('/user') || location.pathname.startsWith('/feedback')
              ...
              <span
                className={`rounded-lg px-3 py-1 text-xs font-semibold uppercase tracking-wide ${
                  isAdminPage || location.pathname.startsWith('/admin/login')
